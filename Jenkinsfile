        ...
        stage('Deploy_Apps') {
            steps {
                script {
                    ...
                    def frontend_image = 'your-dockerhub-account/frontend-image'
                    def backend_image = 'your-dockerhub-account/backend-image'

                    // Pull and run frontend container
                    sh '''
                    ssh -o StrictHostKeyChecking=no -i $PRIVATE_KEY_PATH ubuntu@${frontend_ip} "
                    sudo docker pull ${frontend_image} &&
                    sudo docker run -d -p 80:80 ${frontend_image}
                    "
                    '''
                    ...
                    sudo docker run -d -p 5432:5432 ${backend_image}
                    "
                    '''
                }
            }
